 ...
 */
export const UserProfileRateLimit = () => RateLimit({
  name: 'user-profile',
  ttl: 60, // 1 minute
  limit: 30,
});
 ...
 */
export const SkipRateLimit = (condition: (req: any) => boolean) => {
 ...
 */
export const UserBasedRateLimit = (config: RateLimitConfig) => {
  return CustomRateLimit(config, {
    customKey: (req) => req.user?.id || req.ip,
  });
};
 ...
 */
export const RateLimitWithMessage = (config: RateLimitConfig, message: string) => {
