...
describe('Rate Limiting Performance Tests', () => {
  ...
  describe('Concurrent Access Performance', () => {
    it('should handle high concurrent request volume efficiently', async () => {
      ...
      const clientPromises = Array(concurrency)
        .fill(0)
        .map(async (_, clientIndex) => {
          const clientRequests = Array(requestsPerClient)
            .fill(0)
            .map((_, reqIndex) =>
              request(app.getHttpServer()).get(
                `/test?client=${clientIndex}&req=${reqIndex}`,
              ),
            );
      ...
      const successfulRequests = flatResults.filter(
        (res) => res.status === 200,
      );
      const rateLimitedRequests = flatResults.filter(
        (res) => res.status === 429,
      );
    ...
    it('should maintain consistent response times under load', async () => {
      ...
      for (let i = 0; i < requestCount; i++) {
        ...
        const _response = await request(app.getHttpServer()).get(
          `/test?iteration=${i}`,
        );
    ...
    it('should handle burst traffic patterns', async () => {
      ...
      for (let burst = 0; burst < burstCount; burst++) {
        ...
        const burstRequests = Array(burstSize)
          .fill(0)
          .map((_, i) =>
            request(app.getHttpServer()).get(`/test?burst=${burst}&req=${i}`),
          );
        ...
        const burstSuccessful = burstResults.filter(
          (res) => res.status === 200,
        ).length;
        const burstRateLimited = burstResults.filter(
          (res) => res.status === 429,
        ).length;

        console.log(
          `Burst ${burst + 1}: ${burstSuccessful} successful, ${burstRateLimited} rate-limited (${(burstEnd - burstStart).toFixed(2)}ms)`,
        );
      ...
      const totalSuccessful = allResults.filter(
        (res) => res.status === 200,
      ).length;
      const totalRateLimited = allResults.filter(
        (res) => res.status === 429,
      ).length;
  ...
  describe('Memory and Resource Usage', () => {
    it('should not cause memory leaks under sustained load', async () => {
      ...
      for (let batch = 0; batch < requestCount / batchSize; batch++) {
        const batchPromises = Array(batchSize)
          .fill(0)
          .map(() => request(app.getHttpServer()).get('/test'));
    ...
    it('should handle rate limit storage efficiently', async () => {
      ...
      const ipPromises = Array(uniqueIPs)
        .fill(0)
        .map(async (_, ipIndex) => {
          const ip = `192.168.1.${ipIndex + 1}`;

          const ipRequests = Array(requestsPerIP)
            .fill(0)
            .map(() =>
  ...
  describe('Rate Limit Algorithm Performance', () => {
    it('should demonstrate sliding window accuracy', async () => {
      ...
      for (let window = 0; window < windows; window++) {
        ...
        const successfulInWindow = windowRequests.filter(
          (r) => r.status === 200,
        ).length;
        const rateLimitedInWindow = windowRequests.filter(
          (r) => r.status === 429,
        ).length;

        console.log(
          `Window ${window + 1}: ${successfulInWindow} successful, ${rateLimitedInWindow} rate-limited`,
        );
      }
    ...
    it('should handle different rate limiting strategies', async () => {
      ...
      for (const strategy of strategies) {
        ...
        const requests = Array(requestCount)
          .fill(0)
          .map(() =>
            request(app.getHttpServer()).get(`/test?strategy=${strategy.name}`),
          );
        ...
        console.log(
          `- Successful: ${successful}, Rate-limited: ${rateLimited}`,
        );
  ...
  describe('Scalability Tests', () => {
    it('should maintain performance with increasing load', async () => {
      ...
      for (let i = 1; i < results.length; i++) {
        ...
        const responseTimeRatio =
          current.avgResponseTime / previous.avgResponseTime;
    ...
    it('should handle gradual load increase gracefully', async () => {
      ...
      while (currentLoad <= maxConcurrency) {
        ...
        const promises = Array(currentLoad)
          .fill(0)
          .map(() => request(app.getHttpServer()).get('/test'));
        ...
        console.log(
          `Load ${currentLoad}: ${successful} successful, ${rateLimited} rate-limited`,
        );
      ...
      expect(lastResult.successful + lastResult.rateLimited).toBe(
        lastResult.load,
      );
    });
  });
  ...
  describe('Edge Case Performance', () => {
    it('should handle rapid sequential requests from single client', async () => {
      ...
      console.log(`- Total time: ${totalTime.toFixed(2)}ms`);
      console.log(
        `- Rate: ${(requestCount / (totalTime / 1000)).toFixed(2)} req/sec`,
      );
    ...
    it('should maintain accuracy under time boundary conditions', async () => {
      ...
      const firstBatch = Array(limit)
        .fill(0)
        .map(() => request(app.getHttpServer()).get('/test'));

      const firstResults = await Promise.all(firstBatch);
      const firstSuccessful = firstResults.filter(
        (r) => r.status === 200,
      ).length;
      ...
      // Send another batch
      const secondBatch = Array(limit)
        .fill(0)
        .map(() => request(app.getHttpServer()).get('/test'));

      const secondResults = await Promise.all(secondBatch);
      const secondSuccessful = secondResults.filter(
        (r) => r.status === 200,
      ).length;

      console.log(
        `Second batch: ${secondSuccessful} successful out of ${limit}`,
      );

      // Both batches should have similar success rates
      expect(Math.abs(firstSuccessful - secondSuccessful)).toBeLessThanOrEqual(
        2,
      );
    });
  });
