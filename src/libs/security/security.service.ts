...
export class SecurityService {
      ...
      origin: (origin: string | undefined, callback: (error: Error | null, allow?: boolean) => void) => {
        ...
        if (isProduction) {
          ...
          if (origin !== corsOrigin && !origin.includes('localhost')) {
            this.logger.warn(`CORS allowing non-configured origin in development: ${origin}`);
          }
          callback(null, true);
        }
      },
   ...
   */
  applySecurityMiddleware(req: Request, res: Response, next: NextFunction): void {
  ...
  private logSecurityEvent(req: Request): void {
    ...
    if (req.get('X-Forwarded-Proto') === 'http' && this.envValidator.get('NODE_ENV') === 'production') {
