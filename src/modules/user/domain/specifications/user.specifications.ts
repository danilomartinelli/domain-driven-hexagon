...
export class UserHasSecurePasswordSpecification extends BaseUserSpecification {
  ...
  private readonly passwordValidator = new OptimizedPasswordValidator(DEFAULT_PASSWORD_CONFIG);
...
export class UserCanChangeRoleSpecification extends BaseUserSpecification {
  ...
  isSatisfiedBy(user: UserEntity): boolean {
    ...
    if (this.performedBy.role === UserRoles.moderator) {
      return user.role === UserRoles.guest && this.newRole === UserRoles.moderator;
    }
...
export class UserAccountCanBeDeletedSpecification extends BaseUserSpecification {
  ...
  isSatisfiedBy(user: UserEntity): boolean {
    ...
    if (this.performedBy.role === UserRoles.admin && user.role !== UserRoles.admin) {
      return true;
    }
...
export class EligibleForRoleUpgradeSpecification extends BaseUserSpecification {
  ...
  constructor() {
    super();
    this.specification = new FullyVerifiedUserSpecification()
      .and(new UserLastLoginRecentSpecification(30)); // Active within 30 days
  }
  
  isSatisfiedBy(user: UserEntity): boolean {
    return this.specification.isSatisfiedBy(user) && user.role === UserRoles.guest;
  }
}
...
export class UserSpecificationFactory {
  ...
  static canPerformAction(action: 'delete' | 'changeRole' | 'resetPassword', context: {
    target?: UserEntity;
    ...
    switch (action) {
      ...
      case 'changeRole':
        if (!context.target || !context.performer || !context.newRole) {
          throw new Error('Target, performer, and newRole required for changeRole action');
        }
        return new UserCanChangeRoleSpecification(context.newRole, context.performer);
